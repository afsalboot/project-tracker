...
export default function ProfileSettings({ initialUser }) {
  ...
  async function saveProfile(event) {
    ...
    setSaving(true);
    const response = await fetch("/api/profile", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(profileForm) });
  ...
  async function changePassword(event) {
    ...
    setSaving(true);
    const response = await fetch("/api/profile/password", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ currentPassword: passwordForm.currentPassword, newPassword: passwordForm.newPassword }) });
    const result = await response.json();
    setSaving(false);
    if (!response.ok) return toast.error(result.message);
  ...
  return (
    <>
      ...
      {dialog === "profile" && <Dialog title="Edit profile" description="Your username is the handle teammates will see when they mention you." onClose={() => setDialog(null)}><form className="space-y-4 p-5" onSubmit={saveProfile}>
        <label><span className="label">Full name</span><input className="field" required minLength="2" maxLength="80" value={profileForm.name} onChange={(event) => setProfileForm({ ...profileForm, name: event.target.value })} /></label>
        <label><span className="label">Username</span><div className="relative"><AtSign className="absolute left-3 top-1/2 -translate-y-1/2 text-neutral-400" size={16} /><input className="field !pl-9" minLength="3" maxLength="30" pattern="[a-z0-9._-]+" placeholder="your.username" value={profileForm.username} onChange={(event) => setProfileForm({ ...profileForm, username: event.target.value.toLowerCase().replace(/[^a-z0-9._-]/g, "") })} /></div><span className="mt-1 block text-xs text-neutral-500">Lowercase letters, numbers, dots, hyphens, and underscores.</span></label>
        <label><span className="label">Email address</span><input className="field" type="email" required value={profileForm.email} onChange={(event) => setProfileForm({ ...profileForm, email: event.target.value })} /></label>
      ...
      {dialog === "password" && <Dialog title="Change password" description="Changing your password signs out other sessions while keeping this session active." onClose={() => setDialog(null)}><form className="space-y-4 p-5" onSubmit={changePassword}>
        <label><span className="label">Current password</span><input className="field" type="password" autoComplete="current-password" required value={passwordForm.currentPassword} onChange={(event) => setPasswordForm({ ...passwordForm, currentPassword: event.target.value })} /></label>
        <label><span className="label">New password</span><input className="field" type="password" autoComplete="new-password" required minLength="8" value={passwordForm.newPassword} onChange={(event) => setPasswordForm({ ...passwordForm, newPassword: event.target.value })} /><span className="mt-1 block text-xs text-neutral-500">At least 8 characters with uppercase, lowercase, and a number.</span></label>
        <label><span className="label">Confirm new password</span><input className="field" type="password" autoComplete="new-password" required minLength="8" value={passwordForm.confirmPassword} onChange={(event) => setPasswordForm({ ...passwordForm, confirmPassword: event.target.value })} /></label>
