...
export default function TaskActionsMenu({ task, canEdit, canDelete, canChangeStatus = true, onEdit, onToggleStatus, onDelete, completedStatus = "Completed" }) {
  ...
  return (
    <details className="relative" data-action-menu>
      ...
      <div className="absolute right-0 z-30 mt-1 w-44 rounded-xl border border-neutral-200 bg-white p-1 shadow-xl">
        ...
        {canEdit && canChangeStatus && <button className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-sm hover:bg-neutral-50" onClick={(event) => run(event, onToggleStatus)}>{task.status === completedStatus ? <RotateCcw size={15} /> : <Check size={15} />}{task.status === completedStatus ? "Reopen" : "Complete"}</button>}
