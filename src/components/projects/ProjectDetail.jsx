...
import { PROJECT_STAGES } from "@/constants/project";
    ...
    <>
      <section className="card p-4 sm:p-5 md:p-7">
        <div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-start">
          <div className="min-w-0"><div className="flex flex-wrap gap-2">{project.isArchived && <Badge className="border-neutral-300 bg-neutral-100 text-neutral-700">Archived</Badge>}<Badge>{project.stage}</Badge><Badge>{project.priority}</Badge><Badge>{project.environment}</Badge></div><h2 className="mt-4 break-words text-xl font-semibold sm:text-2xl">{project.name}</h2><p className="mt-1 break-words text-sm text-neutral-500">{project.clientName || "Personal project"} · {project.zohoProduct}{project.moduleName ? ` · ${project.moduleName}` : ""}</p></div>
          <div className="grid grid-cols-2 gap-2 sm:flex sm:flex-wrap">{canEditProject && <button onClick={() => setEditProjectOpen(true)} className="btn btn-secondary w-full sm:w-auto"><Pencil size={16} />Edit</button>}{canCreateTasks && <button className="btn btn-secondary w-full sm:w-auto" onClick={() => setDialog({})}><Plus size={16} />Add task</button>}{canEditProject && !project.isArchived && <button className="btn btn-primary w-full sm:w-auto" onClick={() => mutate("complete")}><Check size={16} />Complete</button>}{canArchiveProject && <button className="btn btn-secondary w-full sm:w-auto" aria-label={project.isArchived ? "Restore project" : "Archive project"} onClick={() => mutate("archive")}><Archive size={16} />{project.isArchived ? "Restore" : "Archive"}</button>}</div>
        </div>
        ...
        <div className="mt-6 grid grid-cols-2 gap-4 border-t border-neutral-100 pt-5 text-sm lg:grid-cols-5"><Info label="Start date"><DateText value={project.startDate} /></Info><Info label="Due date"><DateText value={project.dueDate} /></Info><Info label="Project types">{project.projectTypes?.join(", ") || "Not specified"}</Info><Info label="Assigned users"><AssigneeSummary users={project.assignedUserIds} /></Info><Info label="Last updated">{new Date(project.updatedAt).toLocaleDateString()}</Info></div>
      </section>
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-[1fr_340px]">
        <div className="space-y-5">
          ...
          {canViewTasks && <section>
            <div className="mb-3 grid grid-cols-2 gap-3 sm:flex"><label className="relative col-span-2 flex-1"><Search className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-neutral-400" size={17} /><input className="field search-field" placeholder="Search tasks…" aria-label="Search tasks" value={search} onChange={(event) => setSearch(event.target.value)} /></label><Dropdown multiple className="sm:w-52" ariaLabel="Filter task status" value={statusFilter} onChange={setStatusFilter} placeholder="All statuses" options={TASK_STATUSES} />{canCreateTasks && <button className="btn btn-primary shrink-0" onClick={() => setDialog({})}><Plus size={16} />Add task</button>}</div>
            {visibleTasks.length ? <div className="space-y-3">{visibleTasks.map((task) => <TaskRow key={task._id} task={task} update={updateTask} edit={() => setDialog(task)} expanded={detailsTask?._id === task._id} toggle={() => setDetailsTask(detailsTask?._id === task._id ? null : task)} remove={requestTaskDelete} permissions={permissions} onChanged={load} canEdit={canEditTasks} canDelete={canDeleteTasks} />)}</div> : <EmptyState title="No tasks found" description="Add the first task or change the current filters." />}
          </section>}
        </div>
        <aside className="space-y-5">
          <section className="card p-5"><h3 className="font-semibold">Project stage</h3><Dropdown className="mt-3" disabled={!canEditProject} value={project.stage} onChange={(stage) => mutate("stage", { stage })} options={PROJECT_STAGES} /></section>
        </aside>
      </div>
...
function TaskRow({ task, update, edit, expanded, toggle, remove, permissions, onChanged, canEdit, canDelete }) {
  return (
    <article className="card p-4">
      <div className="grid grid-cols-[minmax(0,1fr)_44px] items-center gap-3 sm:grid-cols-[minmax(0,1fr)_auto_44px]">
        <div className="min-w-0">
          ...
          {task.status === "Blocked" && task.blockerReason && <p className="mt-2 line-clamp-2 text-xs text-red-600">{task.blockerReason}</p>}
        </div>
        <div className="row-start-2 flex flex-wrap items-center gap-3 sm:row-auto"><Badge>{task.status}</Badge><AssigneeSummary users={[task.userId]} empty="Creator unavailable" />{!task.isCreator && <span className="rounded-full bg-neutral-100 px-2.5 py-1 text-xs font-medium text-neutral-500">View only</span>}</div>
        <div className="col-start-2 row-start-1 sm:col-start-3 sm:row-auto">{task.isCreator && <TaskActionsMenu task={task} canEdit={canEdit} canDelete={canDelete} onEdit={edit} onToggleStatus={() => update(task._id, task.status === "Completed" ? "To Do" : "Completed")} onDelete={() => remove(task)} />}</div>
      </div>
